...
export default function EmployeePulsePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // ✅ GET ORG FROM LOGGED-IN USER (THIS IS THE FIX)
        ...
        const orgId = meJson?.organisation_id || null;

        if (!orgId) {
          throw new Error("No organisation linked to this employee profile.");
        }
  ...
  return (
    <div style={{ padding: "2rem" }}>
      ...
      {!loading && success && (
        <p style={{ color: "var(--yellow)", fontWeight: 700 }}>{success}</p>
      )}
