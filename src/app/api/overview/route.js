...
// 🔐 Single-org MVP: hard-wire HRI org
...
export async function GET() {
  try {
    ...
    // 2️⃣ Latest org metrics for THIS org only
    let orgMetrics = null;
    ...
    // 3️⃣ Scores – one row per pillar for this assessment
    ...
    // 6️⃣ Employee pulse summary (latest pulse) – still global for now
    ...
    if (!latestPulseError && latestPulseRow?.pulse_id) {
      ...
      if (!pulseRespError && pulseResponses && pulseResponses.length > 0) {
        const questionIds = [
          ...new Set(
            pulseResponses
              .map((r) => r.question_id)
        ...
        if (questionIds.length > 0) {
          ...
          if (!pulseQError && pulseQuestions && pulseQuestions.length > 0) {
