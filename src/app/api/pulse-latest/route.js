...
export async function GET(req) {
  try {
    ...
    const orgIdParam = urlObj.searchParams.get("organisation_id") || urlObj.searchParams.get("organization_id");
    ...
    if (user && !userError) {
      ...
      return NextResponse.json({ ok: true, organisation_id: organisationId, latest: data || null });
    }

    // 2) No session: allow fallback for testing ONLY if org id provided
    if (!orgIdParam) {
      return NextResponse.json(
    ...
    if (!supabaseUrl || !adminKey) {
      return NextResponse.json(
        { ok: false, error: "Missing env vars", need: ["NEXT_PUBLIC_SUPABASE_URL", "SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY)"] },
        { status: 500 }
      );
    }
    ...
    return NextResponse.json({ ok: true, organisation_id: String(orgIdParam), latest: data || null });
  } catch (e) {
