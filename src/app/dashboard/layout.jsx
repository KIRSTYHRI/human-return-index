...
export default function LoginPage() {
  ...
  const DEBUG_MODE = true; // keep true until you're stable
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data, error }) => {
      ...
      if (data?.session && !DEBUG_MODE) router.replace("/dashboard");
    });
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (mode === "password") {
        ...
        setMsg("Login OK ✅ Session exists.");
        if (DEBUG_MODE) {
          setMsg("Login OK ✅ Session exists. (Debug mode: not redirecting for 2 seconds)");
