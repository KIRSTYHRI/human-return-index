...
export default function DashboardPage() {
  ...
  // Defaults for quick ROI card
  const employeeCount = orgMetrics?.employee_count ?? null;
  const totalPayroll = roiSummary?.total_payroll ?? null;
  const defaultRevenuePerEmployee =
    employeeCount && totalPayroll
      ? Math.round(totalPayroll / employeeCount)
      : 150000;
      ...
      >
        ...
        >
            ...
            >
              ...
              >
                <div>
                  ...
                  >
                    Average scores across your five HRI pillars, all normalised
                    to a 0–100 scale.
                  </p>
                </div>
              </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            {/* HRI Score → £ Impact quick win */}
            <QuickImpactCard
              overallScore={
                overview.overall_score != null
                  ? Number(overview.overall_score)
                  : null
              }
              employeeCount={employeeCount}
              initialRevenuePerEmployee={defaultRevenuePerEmployee}
            />
          </div>
        </div>
        ...
        >
          ...
          {pulseItems.length === 0 ? (
            <p style={{ fontSize: 12, color: "#9ca3af", marginTop: 4 }}>
              No pulse responses yet. Once employees complete a pulse, you'll
              see a live comparison here.
            </p>
          ) : (
            <div
              style={{
...
/** QUICK IMPACT CARD – HRI SCORE → £ IMPACT **/
function QuickImpactCard({
  overallScore,
  employeeCount,
  initialRevenuePerEmployee,
}) {
  const safeInitialRev =
    initialRevenuePerEmployee && Number.isFinite(initialRevenuePerEmployee)
      ? initialRevenuePerEmployee
      : 150000;
  const [improvementPct, setImprovementPct] = useState(10);
  const [revenuePerEmployee, setRevenuePerEmployee] =
    useState(safeInitialRev);

  const pct = Number(improvementPct) || 0;
  const rev = Number(revenuePerEmployee) || 0;
  const people = Number(employeeCount) || 0;

  const perEmployeeImpact = (pct / 100) * rev;
  const totalImpact = perEmployeeImpact * people;
  ...
  return (
    ...
    >
      ...
      >
        Play with the numbers your board cares about. Change the HRI score,
        revenue per employee or % uplift to see the potential value of moving
        your Human Return Index.
      </p>
      ...
      >
        ...
        <div>
          ...
          >
            {people ? people.toLocaleString("en-GB") : "–"}
          </div>
        </div>
      </div>
      ...
      >
        <div>
          ...
          >
            {perEmployeeImpact > 0
              ? `£${Math.round(perEmployeeImpact).toLocaleString("en-GB")}`
              : "–"}
          </div>
        </div>
        ...
        <div>
          ...
          >
            {totalImpact > 0
              ? `£${Math.round(totalImpact).toLocaleString("en-GB")}`
              : "–"}
          </div>
        </div>
      </div>
